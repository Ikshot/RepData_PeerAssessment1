...
MeanStepsPerIntervalPlot<-function(arg_data,plot_color,xlab,ylab)
{
    ...
    for(i in interval_iter)
        interval_steps_over_days[i]=mean(arg_data[arg_data$interval==intervals[i],"steps"],na.rm=TRUE)
    plot(intervals, interval_steps_over_days, type="l", col=plot_color,xlab=xlab,ylab=ylab)
    return(interval_steps_over_days)
}
interval_steps_over_days=MeanStepsPerIntervalPlot(data,"darkblue","Time Intervals","Mean Number of Steps")
```
